...
class QueryNormalizer:
    ...
    def _llm_fuzzy_correct(self, query: str) -> str:
        ...
        system_prompt = (
            ...
            "--- CORRECTION STRATEGY ---\n"
            "1. **Acronym Bidirectional Mapping**: Always equate acronyms and full forms:\n"
            "   - 'SIP' <-> 'Systematic Investment Plan'\n"
            "   - 'NAV' <-> 'Net Asset Value' <-> 'Unit Price'\n"
            "   - 'AUM' <-> 'Assets Under Management' <-> 'Fund Size'\n"
            "   - 'Expense Ratio' <-> 'TER' <-> 'Total Expense Ratio' <-> 'Charges'\n"
            "   - 'Inception' <-> 'Launch Date' <-> 'Start Date'\n"
            "2. **Partial Form Repair**: Fix broken or partial acronyms (e.g., 'systematic plan' -> 'SIP', 'asset value' -> 'NAV').\n"
            "3. **Typos**: Repair phonetic errors ('syp' -> 'SIP', 'nva' -> 'NAV', 'expnse' -> 'Expense Ratio').\n"
            "4. **Scheme Alignment**: Map partial names to canonical forms: 'Small Cap', 'Ethical', 'Multi Asset', 'Flexi Cap', 'Gold ETF', 'Liquid', 'Arbitrage'.\n"
            "5. **Hinglish/Informal**: If a user asks 'how much charges' or 'what is the price', map to 'Expense Ratio' or 'NAV' respectively.\n\n"
            "--- EXAMPLES ---\n"
            "Input: 'What is the systematic investment plan for flexicp?' -> Output: 'minimum SIP for Flexi Cap Fund'\n"
            "Input: 'Net asset value of ethical scheme' -> Output: 'NAV of Ethical Fund'\n"
            "Input: 'how much are the charges for liquid' -> Output: 'expense ratio for Liquid Fund'\n"
            "Input: 'total assets of multi asset' -> Output: 'AUM of Multi Asset Allocation Fund'\n"
            "Input: 'when did the small cap start' -> Output: 'inception date of Small Cap Fund'\n"
            "Input: 'details of syp' -> Output: 'SIP details'\n\n"
            "--- RULES ---\n"
